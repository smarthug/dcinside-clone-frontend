      ...
      <header className="border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-4 py-4">
          <Link to="/" className="flex items-center gap-2 font-semibold tracking-tight">
            <span className="text-xl text-blue-900">KICA</span>
            <span className="text-sm text-slate-500">한국건설감정사회</span>
          </Link>
            ...
            {NAV_LINKS.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  [
                    "transition-colors",
                    isActive ? "text-blue-900" : "text-slate-600 hover:text-blue-800",
                  ].join(" ")
                }
                end={item.to === "/"}
              >
  ...
  return (
    <footer className="mt-16 bg-slate-900 text-slate-200">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-10 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-blue-200">KICA</p>
          <h2 className="text-2xl font-semibold">한국건설감정사회</h2>
          <p className="mt-3 text-sm text-slate-400">
            서울특별시 강동구 성안로 18, 917호 | Tel. 02-6953-1492 | Fax. 02-6919-4008
          </p>
        </div>
        <div className="grid gap-2 text-sm text-slate-300">
          <p>사업자등록번호 560-82-00847</p>
          <p>이메일 kica0472@naver.com</p>
          <p>© {new Date().getFullYear()} Korea Institute of Construction Appraisers</p>
        </div>
      </div>
    </footer>
  );
